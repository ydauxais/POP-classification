...
def main():
    ...
    for file in dataset.db:
        print("CLASS: " + file)
        opt = episode_mining_options()

        pos = rpo.ClassVerticalDatabase(dataset.db[file])
        neg = [[x for other in dataset.db if not other == file
                  and len(dataset.db[other]) > item
                  for x in dataset.db[other][item]]
                    for item in range(len(dataset.db[file]))]
        ...
        opt.minimal_support_ = int(len(dataset.db[file][0]) * args.fmin)
        rpo.run(opt)
        
        for episode in opt.get_episodes():
            print(episode)
